...
PRICE IN RENEWAL / UPCOMING / LIFECYCLE EMAILS
- Renewal reminders, "upcoming charge", and receipt emails frequently state the price plainly. When the email says the customer will be or has been charged a specific amount — patterns like "you'll be charged $X on <date>", "renews at $X/mo", "auto-renews at $X", "your plan renews for $X", "Plan Price: $X", "your card will be charged $X", "Amount paid $X", "$X due on <date>" — set amount: copy that figure into "value" and the FULL literal sentence containing it into "quote". This applies even when eventType is "upcoming" or "started", as long as the price is explicitly stated in THIS email.
- The captured figure MUST be the recurring plan price the customer pays, sitting next to charge/renew/plan-price language. If the email states a recurring price AND a separate one-time/proration/tax/monthly-vs-annual figure, choose the recurring plan price tied to "renews", "per month", "per year", "Plan Price", "/mo", "/yr" — not the proration. If it is genuinely ambiguous which figure is charged, set amount to null.
- Never invent or compute a price. If the email only teases value ("save $X", "a $X value", "$X off", "orders over $X", "Final hours", "your reset starts here") or names no charge amount, set amount to null. Marketing or activation emails that state no charge price MUST have amount null even if you are confident the service exists; a missing price is correct and expected.
- "quote" must appear verbatim in this email body; if you cannot quote the price's sentence verbatim, set amount to null.
